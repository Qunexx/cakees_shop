                ...
                <div class="row mt-5">
                    @foreach($favorites as $favorite)
                        <div class="col-sm-6 col-md-4 col-lg-3 mb-4" id="item-{{ $favorite->id }}">
                            <div class="box">
                                <div class="img-box">
                                    <img src="{{ asset('images/' . $favorite->product->image) }}" alt="{{ $favorite->product->name }}"
                                ...
                                <div class="detail-box">
                                    ...
                                    <form action="{{ route('favorites.remove',$favorite->product->id)}}" method="POST">
                                        ...
                                        @method('DELETE')
                                        <input type="hidden" name="product_id" value="{{$favorite->product->id}}">
                                        <button type="submit" class="btn btn-danger remove-favorite-btn" data-id="{{ $favorite->id }}">Удалить из избранного</button>
                                    </form>
                                    </div>
                                </div>
                            </div>
